...
public class CriticalDebuggingTest : IDisposable
{
    ...
    public CriticalDebuggingTest()
    {
        ...
        // CRITICAL FIX: Use extremely small page size to force one object per page for debugging
        innerStorage.Initialize(_testRootPath, new StorageConfig { 
            Format = SerializationFormat.Json,
            MaxPageSizeKB = 1  // This is actually too large - need to fix the logic
        });
    ...
    public void CriticalDebug_TwoSimpleUpdates_TraceDataLoss()
    {
        ...
        // Update 1: Update object with Id=1 in page1
        Console.WriteLine("=== UPDATE 1: Updating object with Id=1 in page1 ===");
        ...
        // CRITICAL FIX: Preserve other objects when updating
        var updatedPage1Content = new List<object>();
        foreach (dynamic obj in page1Data)
        {
            if ((int)obj.Id == 1)
            {
                // Update object with Id=1
                updatedPage1Content.Add(new { Id = 1, Value = "Updated_Object1", UpdatedAt = DateTime.UtcNow });
            }
            else
            {
                // Keep other objects unchanged
                updatedPage1Content.Add(obj);
            }
        }
        
        _storage.UpdatePage(update1Txn, "critical.test", page1, updatedPage1Content.ToArray());
        Console.WriteLine("Updated object with Id=1 while preserving others");
        ...
        // Update 2: Update object with Id=2 in page2 (which is actually the same page as page1)
        Console.WriteLine("=== UPDATE 2: Updating object with Id=2 in page2 ===");
        ...
        // CRITICAL FIX: Preserve other objects when updating
        var updatedPage2Content = new List<object>();
        foreach (dynamic obj in page2Data)
        {
            if ((int)obj.Id == 2)
            {
                // Update object with Id=2
                updatedPage2Content.Add(new { Id = 2, Value = "Updated_Object2", UpdatedAt = DateTime.UtcNow });
            }
            else
            {
                // Keep other objects unchanged
                updatedPage2Content.Add(obj);
            }
        }
        
        _storage.UpdatePage(update2Txn, "critical.test", page2, updatedPage2Content.ToArray());
        Console.WriteLine("Updated object with Id=2 while preserving others");
